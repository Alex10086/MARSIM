...
```bash
# 先 launch 全栈，然后：
ros2 run marsim_nav nav_diag -- -28 -12          # 飞完并输出全套指标（默认 240s）
ros2 run marsim_nav nav_diag -- --plan -28 -12   # 只取首条计划算间距，不起飞（~15s）
```

`nav_diag` 一次给出：计划间距与**未用余量**、指令 vs 实际速度、相对**当刻**计划的横向偏差、
轨迹到真实障碍的最小间距、以及按 20s 分档的 `|cmd|` / `wz` / `yaw` /
「机头与运动方向夹角」/ 机体处局部代价 / 走过路程。

> `--plan` 模式的存在理由：为了知道某个**规划器**参数有没有用而飞完整条路线要几分钟，
> 而计划在目标被接受后一秒内就有了。调规划器参数请用它。
...
实测（启动后顺序发 4 个目标，等每个真正结束再发下一个）：

```
(-28,-12) 295s (0.11 m/s)   <- 第一个！
( 20,-10)  99s (0.49 m/s)
( 36,  0)  38s (0.49 m/s)
( 44,-10)  23s (0.54 m/s)
```

用 `nav_diag` 排除掉的可能：**不是慢热**（300 秒全程 0.11，只有三段短暂冲到 0.5）、
**不是 `quad_pid`**（实际速度 = 指令速度）、**不是局部代价图**（机体处 cost 0~36≈自由）、
**不是「转不过来」**（`wz ≈ 0`，MPPI 压根不下旋转指令）。
异常的是 `Δyaw` 长期 30~105°——**机头与运动方向差很远，无人机在侧着飞**。

当前怀疑（**尚未验证**）：`PathAlignCritic`（权重 14，全场最高）累计「机头与路径方向偏差」，
而该代价随**走过的距离**累积，于是 MPPI 的理性选择是**少走**（慢）。
注意：验证它必须**在第一个目标上测**——把它降到 6 曾在 `(44,-10)` 上测过，
而那条路线本来就有 0.5 m/s，根本看不出效果（**这是我自己踩过的坑**）。
